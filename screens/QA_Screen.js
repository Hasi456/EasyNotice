...
render(){
    ...
    return (
      ...
      <ScrollView>
        ...
        {this.state.dataList.map((item, key) => ((item.respond!='')?
          <View key={key} style={{backgroundColor:'#81F79F', borderColor:'black', borderWidth:2,margin:5}}>
            <View style={{flexDirection:"row"}}>
              <Text>{item.ndate}</Text>
              <Text style={{marginLeft:230}}>{item.ntime}</Text>
              </View>
            
            <Text>from admin (privately):-</Text>
            <Text style={styles.text,{fontWeight:"bold", fontSize:20, textAlign:'center'}}>{item.title}</Text>
